...
export const useProjects = () => {
  ...
  // Find project by ID
  const findProjectById = useCallback((id: string): Project | undefined => {
    return projects.find(project => project.uuid === id);
  }, [projects]);

  // Get project analysis data (mock implementation)
  const getProjectAnalysis = useCallback(async (id: string): Promise<TranscriptionAnalysis | null> => {
...
Are there any questions about our performance this quarter or our plans moving forward?`,
      ...
      analysis: {
        overall_assessment: {
          score: 0.82,
          key_insights_summary: 'The presentation demonstrates strong storytelling abilities and effective use of data to support key points. The speaker shows good energy and engagement with the audience throughout.',
          areas_for_improving: [
        ...
        presentation_breakdown: {
          storytelling_coherence: {
            score: 0.85,
            assessment: 'Excellent narrative flow with clear beginning, middle, and end. Good use of specific examples like TechCorp Industries case study.',
            suggestions: [
          ...
          listener_motivation: {
            score: 0.78,
            assessment: 'Strong motivational content with concrete results and forward-looking vision. Could benefit from more audience interaction.',
            suggestions: [
          ...
          closing_statement_engagement: {
            score: 0.65,
            assessment: 'The closing asks for questions but lacks a strong call-to-action or memorable final statement.',
            suggestions: [
        ...
        emotional_analysis: {
          identified_emotions: [
            {
              emotion: 'Confidence',
              quotes: 'I\'m excited to share our Q2 sales performance and strategies that have led to our success'
            },
            {
              emotion: 'Pride',
              quotes: 'However, how we responded to these challenges is what I\'m most proud of'
            },
            {
              emotion: 'Optimism',
              quotes: 'Looking ahead to Q3, I\'m excited about the opportunities in front of us'
            },
            {
              emotion: 'Honesty',
          ...
          overall_emotional_arc: 'The presentation follows a positive emotional journey, starting with confidence and excitement, acknowledging challenges with honesty, and ending on an optimistic note about the future.'
        }
      }
    };
  ...
  const addProject = useCallback(async (projectData: CreateProjectData): Promise<Project> => {
    ...
    try {
      ...
      const newProject: Project = {
        uuid: 'project-' + Date.now() + '-' + Math.random().toString(36).substr(2, 9),
        ...projectData,
  ...
  const pendingProjects = useMemo(() => 
    projects.filter(p => p.status === 'PENDING'), [projects]
  );
